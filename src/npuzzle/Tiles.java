...
//		if (tiles.hasCode() == hasCode()) {
//			return true;
//		}

		boolean equalTiles = false;
		if (this.tiles.length != tiles.tiles.length) {
			equalTiles = false;
		} else {
			int i = 0;
			for (; i < this.tiles.length; i++) {
				if (this.tiles[i] != tiles.tiles[i]) {
					equalTiles = false;
					break;
				}
			}
		}
 		return equalTiles && width == tiles.width &&
				emptyTileColumn == tiles.emptyTileColumn &&
				emptyTileRow == tiles.emptyTileRow;
	}
